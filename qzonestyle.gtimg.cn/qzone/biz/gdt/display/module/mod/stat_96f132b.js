...
GDTDefine('gdt:mod/stat.js', function(require, exports, module) {
    ...
    StatMod.sendErrMsg = function(e, url, pid, opt) {
        ...
        data = {dataId: 1000058,bid: 20,url: comm.string.cut(url, 128 - charleft),lineNo: 0,browser: browserinfo.broswer,errMsg: [pid, orderdesc, msg, navigator.userAgent]};
        if (!!stack) {
        ...
        setTimeout(function() {
            if (extra && comm.TCISD) {
                var opts = {referURL: 'http://qzone.com/appinfo',referDomain: 'qzone.com',referPath: '/appinfo'}, emsg = comm.string.cut('/' + ([pid, msg, stack]).join('').replace(/["\/:]/g, ''), 256);
        ...
        if (opt.resp) {
            ...
            StatMod._postdata(errcgi, {jsondata: '【' + errMsg.join('】 【') + '】'});
        }
    };
    ...
    StatMod.sendStat = function(errcode, startTime, calledlist, opts) {
        ...
        function _send(pid) {
            ...
            if (typeof errstatus != 'undefined') {
                var map = {chrome: 1000,firefox: 2000,ie: 3000,opera: 4000,safari: 5000}, binfo, bcode;
            ...
            urlext = '?' + pid;
            url = config._burl;
            helper.valueStat(url, succode, ecode, duration, rate, {urlext: urlext});
            if (succode != 2) {
                urlext = '?qzfl';
                helper.valueStat(url, succode, ecode, duration, rate, {urlext: urlext});
            }
    ...
    StatMod.reportReqErr = function(e, pid, hasOrder) {
        try {
            ...
            if (r < 1 || config._full_stat) {
                StatMod.sendErrMsg(e, url, pid, {extra: extra,hasOrder: hasOrder});
            }
        } catch (ex) {
